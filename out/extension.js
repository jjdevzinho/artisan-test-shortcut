...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand("artisanTest.runTest", () => {
        ...
        if (taskName === "Artisan Test") {
            vscode.window
                ...
                if (selection) {
                    ...
                    const vscodeTask = new vscode.Task({ type: task.type, task: task.label }, vscode.TaskScope.Workspace, task.label, task.type, new vscode.ShellExecution(task.command, args), task.problemMatchers);
            ...
            vscode.window
                ...
                .then((filterTag) => {
                if (filterTag) {
                    const vscodeTask = new vscode.Task({ type: task.type, task: task.label }, vscode.TaskScope.Workspace, task.label, task.type, new vscode.ShellExecution(task.command, [
                        "artisan",
                        "test",
                        "--filter",
                        filterTag,
            ...
            const vscodeTask = new vscode.Task({ type: task.type, task: task.label }, vscode.TaskScope.Workspace, task.label, task.type, new vscode.ShellExecution(task.command, task.args), task.problemMatchers);
            vscode.tasks.executeTask(vscodeTask);
        }
    }
    else {
